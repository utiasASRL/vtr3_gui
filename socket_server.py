...
##### VTR specific calls #####
# def defaultValueForDict(d, key, default):
#   if key not in d:
#     return default
#   else:
#     return d
...
def add_goal(json):
    ...
    print(json)
    print("Type:" + str(type(json)))
    goal_str = json.get('type', None)
    print(json.get('la', "0.0"))
    """
  if goal_str is None:
  ...
  return True, goal_id
  """
    return True, "f9c8e7a2-f9c8-f9c8-f9c8-f9c8e7a2f9c8"
...
def broadcast_robot(
  ...
  """
    ...
    socketio.emit(u"robot/loc", bytes(status.SerializeToString()),
                  broadcast=True)
